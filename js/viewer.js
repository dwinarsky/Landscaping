...
export class Viewer {
  ...
  _bind() {
    ...
    map.addEventListener("pointerdown", (e) => {
      ...
      // Deliberately not capturing yet. A captured pointer makes the browser
      // fire the following click at the capture element, so capturing here
      // would send every tap to the map and the zone and marker handlers would
      // never run - the same retargeting the .mapctl guard above avoids.
      // Capture happens in pointermove, once this is definitely a drag.
    ...
    map.addEventListener("pointermove", (e) => {
      ...
      if (Math.abs(dx) + Math.abs(dy) > 2) {
        this.moved = true;
        // Now that it is a drag, capture so panning survives the pointer
        // leaving the map. A tap never reaches here, so its click is intact.
        this._capture(e.pointerId);
      }
    ...
    this._capture = (id) => {
      if (map.hasPointerCapture(id)) return;
      // The pointer can be gone already if the gesture ended mid-move.
      try { map.setPointerCapture(id); } catch { /* nothing to capture */ }
    };
